...
if (!browserStorage) {
  ...
  objectLength(JSON.parse(browserStorage)) !== objectLength(questionSet)
) {
...
function calculateProportion(s) {
  ...
  document.getElementById(`percent${s.id}`).innerHTML =
    Math.floor(proportion * 100) + "%";
  document.getElementById(`circle${s.id}`).style.strokeDashoffset = Math.floor(
    260 - 260 * proportion
  );
}
...
function listOfQuestions(id) {
  ...
  for (let key in questions) {
    ...
    setOfQuestions.innerHTML += `<div class="col-md-6 col-lg-4 col-xl-3">
        ...
        <p class="set-title m-0">${functionName.slice(
          0,
    ...
    if (question.solved)
      document.getElementById(`solved${key}`).classList.remove("d-none");
  }
}
...
function questionWorkspace(id, key) {
  ...
  <div class="buttons mb-4">
    ...
    <button class="btn mb-2 mb-sm-0" onclick="questionWorkspace(${id}, ${
    key != 1 ? key - 1 : 1
  })">Oldingisi</button>
    <button class="btn mb-2 mb-sm-0" onclick="questionWorkspace(${id}, ${
    key != objectLength(obj.questions) ? key + 1 : key
  })">Keyingisi</button>
  </div>
  ...
  if (!localStorage.getItem("" + id + key)) {
    ...
    document.getElementById("answer").value = localStorage.getItem(
      "" + id + key
    );

  document
    .getElementById("submit")
    .setAttribute("data-question", `${id} ${key}`);
  update();
}
